const validateBlogData = ({title,textBody})=>{
    return new Promise ((res,rej)=>{
        ...
        if(title.length <3 ||title.length>100) return rej("character length should be between 3-100 characters of Title");
        if(textBody.length < 3 || textBody.length>1000) return rej("character length should be between 3-1000 characters of Title")
